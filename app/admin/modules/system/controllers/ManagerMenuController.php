...
class System_ManagerMenuController extends Admin_Controller_Action
{
    ...
    function indexAction(){
        $this->view->requestParams = $requestParams = $this->getRequest()->getParams();
        $serviceManagerMenu = new Admin_Service_ManagerMenu();
        if(($requestParams['menu_detail'] == 'yes')&&(!empty($requestPaams['menu_id']))){
            if(!$this->view->detail = $detail = $serviceManagerMenu->getDetailById($requestParams['menu_id'])){
                $this->setMsgs($serviceManagerMenu->getMsgs());
            }
        }
        if ($requestParams['data_ajax'] == 'treegrid_data') { // 获取数据
