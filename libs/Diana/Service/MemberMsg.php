...
class Diana_Service_MemberMsg extends Diana_Service_Abstract
{
    ...
    function draft($param,$msgId = null)
    {
        ...
        if((!empty($msgId))&&(is_numeric($msgId))){
            ...
            if(!empty($tmpRowsMemberMsg[0]['msg_send_time'])){
        ...
        if(!$rowsMemberMsg = $modelMemberMsg->write($data,$msgId)){
            $this->setMsgs('消息写入失败');
            return false;
        }
        $rowMemberMsg = $rowsMemberMsg[0];
        return $rowMemberMsg;
    }
    ...
    function send($data,$id = null,$isSystem = null)
    {
        ...
        if(empty($isSystem)){//不是系统邮件就需要限制发送数据
            ...
            if($configMessageSendCountMember < $countWithOutboxByMnanager ){
