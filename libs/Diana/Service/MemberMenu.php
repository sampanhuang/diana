 ...
 */
class Diana_Service_MemberMenu extends Diana_Service_Abstract
{
    ...
    function insert($input)
    {
        ...
        if(!$rowsMemberMenu = $modelMemberMenu->insert($input,$this->sessionMember)){
            $this->setMsgs('保存失败');
            return false;
        }
    ...
    function update($input,$menuId)
    {
        ...
        $modelMemberMenu = new Diana_Model_MemberMenu();
        if(!$rowsMemberMenu = $modelMemberMenu->update($input,$menuId,$this->sessionMember)){
            $this->setMsgs('保存失败');
            return false;
        }
        return $rowsMemberMenu[0];
