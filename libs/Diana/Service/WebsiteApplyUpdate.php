 ...
 */
class Diana_Service_WebsiteApplyUpdate extends Diana_Service_Abstract
{
    ...
    function postApply($data)
    {
        ...
        //写入纪录
        $modelWebsiteApplyUpdate = new Diana_Model_WebsiteApplyUpdate();
        if(!$rowsWebsiteApply = $modelWebsiteApplyUpdate->register($memberId,$data['website_name'],$data['website_domain'],$data['website_tag'],$data['website_categoryId'],$data['website_continent'],$data['website_country'])){
            //删除会员数据
            if($rollbackMember == 1){
        ...
        //添加网站介绍
        $modelWebsiteApplyUpdateIntro = new Diana_Model_WebsiteApplyUpdateIntro();
        if(!$modelWebsiteApplyUpdateIntro->saveIntro($applyId,$data['website_intro'])){
            //删除提交数据
            $modelWebsiteApplyUpdate->deleteById($applyId);
            //删除会员数据
            if($rollbackMember == 1){
                $modelMember->deleteById($memberId);
            }
        ...
        if(!$modelWebsiteTrendApply->update()){
            ...
            $modelWebsiteApplyUpdate->deleteById($applyId);
            //删除网站描述
            $modelWebsiteApplyUpdateIntro->clearIntro($applyId);
            //删除会员数据
            if($rollbackMember == 1){
                $modelMember->deleteById($memberId);
            }
    ...
    function accedeApply($applyId)
    {
        ...
        //获取他们的简介
        $modelWebsiteApplyUpdateInstro = new Diana_Model_WebsiteApplyUpdateIntro();
        if(!$optionsWebsiteApplyInstro = $modelWebsiteApplyUpdateInstro->getIntroById(null,$applyId)){
            return false;
        }
        //已经存在重复网站名和域名的申请ID
        $existApplyId = array();
        ...
        $modelWebsiteApplyUpdate = new Diana_Model_WebsiteApplyUpdate();
        if(!empty($okApplyId)){
            $modelWebsiteApplyUpdate->updatePass($okApplyId,1);
        }
        //已经存在的就没有办法通过审核
        if(!empty($existApplyId)){
            $modelWebsiteApplyUpdate->updatePass($existApplyId,2);
        }
        //更新网站注册动态
    ...
    function demurApply($applyId)
    {
        ...
        $modelWebsiteApplyUpdate = new Diana_Model_WebsiteApplyUpdate();
        $modelWebsiteApplyUpdate->updatePass($applyId,2);
    ...
    function checkApplyId($applyId)
    {
        ...
        $modelWebsiteApplyUpdate = new Diana_Model_WebsiteApplyUpdate();
        if(!$rowsWebsiteApply = $modelWebsiteApplyUpdate->getRowsById(null,$applyId)){
            $this->setMsgs('无效的参数');
            return false;
        }
        return $rowsWebsiteApply;
    ...
    function pageByCondition($page = 1,$pagesize = 1,$condition = array(),$order = null)
    {
        ...
        $modelWebsiteApplyUpdate = new Diana_Model_WebsiteApplyUpdate();
        if($countWebsiteApply = $modelWebsiteApplyUpdate->getCountByCondition(null,$condition)){
            if($rowsWebsiteApply = $modelWebsiteApplyUpdate->getRowsByCondition(null,$condition,null,$pagesize,$offset)){
                $memberIds = array();
                foreach($rowsWebsiteApply as $rowWebsiteApply){
    ...
    function detailById($applyId)
    {
        ...
        //获取网站详情
        $modelWebsiteApplyUpdateIntro = new Diana_Model_WebsiteApplyUpdateIntro();
        if($rowsWebsiteIntro = $modelWebsiteApplyUpdateIntro->getIntroById(null,$applyId)){
    ...
    function deleteById($applyId)
    {
        ...
        //删除网站简介
        $modelWebsiteApplyUpdateIntro = new Diana_Model_WebsiteApplyUpdateIntro();
        if($rowsWebsiteApplyIntro = $modelWebsiteApplyUpdateIntro->getRowsById(null,$applyId)){
            if(!$modelWebsiteApplyUpdateIntro->delData($condition)){
                foreach($rowsWebsiteApply as $rowWebsiteApply){
                    $modelWebsiteApplyUpdate->saveData(1,$rowWebsiteApply);
                }
                $this->setMsgs('简介删除失败');
                return false;
            }
