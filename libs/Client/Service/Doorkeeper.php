 ...
 *
 */
class Client_Service_Doorkeeper extends Client_Service_Abstract
{
    var $focus = 0;
    ...
    function login($email,$passwd,$captcha)
    {
        ...
        if (!$serviceCaptcha->checkCaptchaWord($captcha,"client-member-login")) {
            $this->focus = 3;
        ...
        if (!$serviceMemberLog = $serviceMemberLog->write(210,$detailMember['member_id'],$detailMember['member_email'],$detailMember['member_name'])) {
    ...
    function checkMember($id = null,$loginUser = null)
    {
        ...
        //能否根据条件得到用户信息
        if (!$detailMember) {
            $this->setMsgs("当前会员帐号【 $id $loginUser 】并不存在");
            return false;
        }
        //判断用户是否被锁定
