...
class Com_IpLocation 
{
	...
	function __construct($datfile)
	{
		...
		$this->totalip =($this->lastip - $this->firstip)/7 ; //ip总数 索引区是定长的7个字节,在此要除以7, php程序员之家 
	 ...
	 * @return unknown
	 */
	function getaddressutf($ip)
	{
		if (!$address = $this->getaddress($ip)) {
			return false;
		}
		$address["area1"] = iconv("gbk",'utf-8',$address["area1"]);
		$address["area2"] = iconv("gbk",'utf-8',$address["area2"]);
		$commonIpCnProvince = new Com_IpCnProvince();
		if ($provinceCode = $commonIpCnProvince->getCodeByAddr($address["area1"])) {
			$commonIpCnCity = new Com_IpCnCity();
			$cityCode = $commonIpCnCity->getCodeByAddr($address["area1"],$provinceCode);
			$countryCode = "cn";
		}else{
			$commonIpCountry = new Com_IpCountry();
			$countryCode = $commonIpCountry->getCodeByAddr($address["area1"]);
		}
