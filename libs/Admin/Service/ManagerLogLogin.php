 ...
 *
 */
class Admin_Service_ManagerLogLogin extends Admin_Service_Abstract
{
    ...
    function write($id,$email,$name)
    {
        ...
        $model = new Diana_Model_ManagerLogLogin();
    ...
    function search($data = array(),$page = 1,$pagesize = 20)
    {
        ...
        if ($total = $modelManagerLogLogin->getCountByCondition(null,$condition)) {
            $return['total'] = $total;
            $count = $pagesize;
            $offset = ($page-1)*$count;
            if ($rows = $modelManagerLogLogin->getRowsByCondition(null,$condition,null,$count,$offset)) {
                $return['rows'] = $rows;
            }
        }
        return $return;
