 ...
 */
class Admin_Service_MemberMenu extends Admin_Service_Abstract
{
    ...
    function insert($input)
    {
        ...
        if(!$rowsMemberMenu = $modelMemberMenu->insert($input,$this->sessionManager)){
            $this->setMsgs('保存失败');
            return false;
        }
        return $rowsMemberMenu[0];
    }
    ...
    function update($input,$menuId)
    {
        ...
        $modelMemberMenu = new Diana_Model_MemberMenu();
        if(!$rowsMemberMenu = $modelMemberMenu->update($input,$menuId,$this->sessionManager)){
            $this->setMsgs('保存失败');
            return false;
        }
        return $rowsMemberMenu[0];
    ...
    function deleteById($menuId)
    {
        ...
        $modelMemberMenu = new Diana_Model_MemberMenu();
        if($rowsMenuSon = $modelMemberMenu->getRowsByFather(null,$menuId)){
            $this->setMsgs('无法删除包含有下级的菜单！请先删除他的下级！');
            return false;
        }
        if(!$rowsAffected = $modelMemberMenu->delData(array('menu_id' => $menuId))){
    ...
    function makeComboTree($tree)
    {
        if((empty($tree))||(!is_array($tree))){
            return array(array('id' => 0,'text' => 'root'));
        }
        $treeGird = array();
    ...
    function makeTreeByIds($menuIds)
    {
        ...
        //获取第一层ID
        if($rowsFather = $modelMemberMenu->getRowsById(null,$fatherMenuIds)){
        ...
        $menuIds = array_filter(array_unique($menuIds));
        //再重新取一次
        if(!$rowsMenu = $modelMemberMenu->getRowsById(null,$menuIds)){
            $this->setMsgs('数据获取失败');
            return false;
        }
